...
def create_bussines(nombre_empresa):
    try:
        ...
        data.execute(f"CREATE table {nombre_empresa} (id_empresa int primary key auto_increment, nombre varchar (100), rnc varchar (50), direccion varchar (300), telefono varchar (50), correo varchar (50), encargado varchar (100), logo varchar (200), fecha_registro timestamp);")
...
def registrar(nombre, rnc, direccion, telefono, correo, encargado):
    try:
        ...
        data.execute(f"INSERT INTO `{nombre}` (`id_empresa`, `nombre`, `rnc`, `direccion`, `telefono`, `correo`, `encargado`) VALUES (NULL, '{nombre}', '{rnc}', '{direccion}', '{telefono}', '{correo}', '{encargado}');")
        con.conexion.commit()
...
print(registrar('SoftFlesh','RDF22222152', 'Calle SAntiago Rodriguez', '809-536-4833', 'Freddypimpns@gmail.com', 'Freddy Pimpns'))
